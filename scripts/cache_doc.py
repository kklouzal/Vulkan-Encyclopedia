...
ALLOWED_NETLOC = 'docs.vulkan.org'
ALLOWED_SCHEMES = {'https'}
ALLOWED_PATH_PREFIX = '/spec/latest'
...
def default_root() -> Path:
    return Path.cwd() / '.Vulkan-Encyclopedia'
...
def validate_url(url: str) -> urllib.parse.ParseResult:
    ...
    if not parsed.path.startswith(ALLOWED_PATH_PREFIX):
        raise ValueError(f'Only {ALLOWED_PATH_PREFIX} URLs are supported')
    return parsed
...
def fetch_html(url: str) -> str:
    req = urllib.request.Request(url, headers={'User-Agent': 'Vulkan-Encyclopedia/1.0'})
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Fetch and cache a Vulkan docs page into .Vulkan-Encyclopedia.')
    parser.add_argument('--url', required=True, help='docs.vulkan.org/spec/latest URL to cache')
    parser.add_argument('--root', default=str(default_root()), help='Vulkan Encyclopedia data root (default: <cwd>/.Vulkan-Encyclopedia)')
    ...
    title = extractor.title.strip() or Path(parsed.path).name or 'Vulkan Doc'
    body = extractor.text()
